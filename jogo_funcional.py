...
def calculate_arm_opening(pose_landmarks, frame_w, frame_h, side):
    """
    Calcula o ângulo de abertura do braço em relação ao corpo (abdução do ombro).
    Usa o ângulo entre o vetor ombro-pulso e a vertical (eixo y).
    Retorna em graus.
    """
    global arm_openings
    if side == 'right':
    ...
    if shoulder.visibility > 0.5 and wrist.visibility > 0.5:
        ...
        vertical_vector = np.array([0, 1])  # Vetor vertical para baixo
        
        # Normaliza vetores
        arm_norm = arm_vector / np.linalg.norm(arm_vector) if np.linalg.norm(arm_vector) > 0 else arm_vector
        vertical_norm = vertical_vector
        
        # Ângulo entre os vetores
        dot_product = np.dot(arm_norm, vertical_norm)
    ...
    return arm_openings[side]

# --- Loop Principal ---
cv2.namedWindow("Fruit Ninja 3D - Duas Mãos", cv2.WINDOW_NORMAL)
cv2.resizeWindow("Fruit Ninja 3D - Duas Mãos", 1280, 720)  # Aumenta o tamanho da janela
